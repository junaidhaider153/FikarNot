export const getProductReviews = (reviews, productId) => reviews.filter((review) => review.productId === productId && review.status === "published").sort((a, b) => b.createdAt - a.createdAt);
...
export const getProductReviewSummary = (reviews, productId, fallbackRating = 0) => {
  ...
  const distribution = items.reduce((acc, item) => { acc[item.rating] = (acc[item.rating] || 0) + 1; return acc; }, { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 });
