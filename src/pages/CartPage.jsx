        ...
        <Empty
          ...
          cta={
            <Link className="btn btn-dark" to="/products">
              Browse the shop <Ic n="arrow" s={15} />
            </Link>
          }
        />
      </div>
      ...
      <div className="cart-layout">
        <section className="cart-lines" aria-label="Cart items">
          <div className="cart-free-shipping">
            <div className="cart-free-head">
              <strong>
                {summary.freeShippingUnlocked
                  ...
                  : `You're ${fmt(summary.amountToFreeShipping)} away from free shipping`}
              </strong>
            ...
            <p>
              {summary.freeShippingUnlocked
                ? "Nice choice. Your order qualifies for free standard shipping."
                : "Spend $75 or more and standard shipping is free."}
            </p>
          </div>

          <div className="cart-item-list">
            {summary.lines.map((line) => (
              <CartItem key={line.p.id} line={line} />
            ))}
          </div>
        </section>
        ...
        <aside className="summary cart-summary">
          ...
          <Link className="btn btn-lime cart-checkout-btn" to="/checkout">
            <span>Checkout</span>
            <span>
              {fmt(summary.total)} <Ic n="arrow" s={15} />
            </span>
          </Link>
          <Link className="btn btn-ghost cart-continue-btn" to="/products">
            Continue shopping
          </Link>
          <div className="cart-trust">
            <div>
              <Ic n="shield" s={15} /> Secure checkout
            </div>
            <div>
              <Ic n="truck" s={15} /> Free shipping over $75
            </div>
            <div>
              <Ic n="check" s={15} /> 30-day returns
            </div>
          </div>
        </aside>
      </div>
