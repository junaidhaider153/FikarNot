...
const couponSeed = [
  { id: "cp1", code: "WELCOME10", type: "percent", value: 10, minSubtotal: 50, maxUses: 100, usedCount: 0, active: 1, expiresAt: Date.now() + 30 * 864e5, description: "10% off orders over $50." },
  { id: "cp2", code: "FREESHIP", type: "free_shipping", value: 0, minSubtotal: 50, maxUses: 0, usedCount: 0, active: 1, expiresAt: Date.now() + 60 * 864e5, description: "Free standard shipping on orders over $50." },
  { id: "cp3", code: "SAVE20", type: "fixed", value: 20, minSubtotal: 120, maxUses: 50, usedCount: 0, active: 1, expiresAt: Date.now() + 45 * 864e5, description: "$20 off orders over $120." },
];
...
const DEFAULT_SITE_SETTINGS = {
  ...
  heroKicker: "Curated essentials · Free shipping over $75",
  ...
  heroImage: "",
  announcement: "Free shipping over $75 · 30-day returns",
...
const sendOrderConfirmationEmail = async (order, items) => sendTransactionalEmail({
  to: order.customer_email,
  subject: `FikarNot order ${order.id} confirmed`,
  html: `<div><h1>Thanks for your order</h1><p>Your order <strong>${escapeHtml(order.id)}</strong> has been received.</p><ul>${items.map((item) => `<li>${escapeHtml(item.name)} × ${item.qty} — $${Number(item.price * item.qty).toFixed(2)}</li>`).join("")}</ul><p><strong>Total: $${Number(order.total).toFixed(2)}</strong></p><p><a href="${APP_ORIGIN}/account">View your orders</a></p></div>`,
  text: `Your FikarNot order ${order.id} is confirmed. Total: $${Number(order.total).toFixed(2)}. View orders at ${APP_ORIGIN}/account.`,
...
const saveUploadedImage = (dataUrl, { uploadedBy = null, originalName = "" } = {}) => {
  ...
  try {
    ...
    try { fs.unlinkSync(filePath); } catch { /* Best-effort cleanup; preserve the original error. */ }
...
const mediaUsageCount = (url) => {
  ...
  for (const row of products) {
    ...
    let images = [];
    try { images = JSON.parse(row.images_json || "[]"); } catch { /* Ignore malformed legacy image metadata. */ }
...
const server = http.createServer(async (req, res) => {
  ...
  if (!verifyCsrf(req, res, csrfCookieValue)) return;
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/orders") {
      ...
      if (!new Set(["cod", "card"]).has(paymentMethod)) {
      ...
      if (paymentMethod === "card" && !MOCK_PAYMENTS_ENABLED) {
        return send(req, res, 503, { error: "PAYMENT_PROVIDER_NOT_CONFIGURED", message: "Card payments are not currently available. Please choose cash on delivery." });
      }
      ...
      try {
        ...
        const now = Date.now();
        db.prepare("INSERT INTO orders (id,user_id,customer_name,customer_email,customer_address,payment_method,subtotal,discount,shipping,total,coupon_json,status,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)")
          .run(id, user?.id || null, String(customer.name || 'Guest').trim(), email, String(customer.address || ''), paymentMethod, subtotal, discount, shipping, total, JSON.stringify(couponResult.coupon ? { code: couponResult.coupon.code, type: couponResult.coupon.type, value: couponResult.coupon.value, discount, shippingFree: couponResult.shippingFree } : null), 'paid', now);
        ...
        if (couponResult.coupon) db.prepare("UPDATE coupons SET used_count=used_count+1 WHERE id=?").run(couponResult.coupon.id);
        db.exec("COMMIT");
        if (user?.id) createNotification(user.id, { type: 'order', title: `Order ${id} confirmed`, message: 'Thanks for your order. Your items are being prepared.', link: '/account', orderId: id });
        const row = db.prepare("SELECT * FROM orders WHERE id=?").get(id);
        void sendOrderConfirmationEmail(row, items);
        return send(req, res, 201, { order: orderRow(row, items) });
      } catch (e) {
        db.exec("ROLLBACK");
        const status = ["INVALID_COUPON","COUPON_MIN_SUBTOTAL","PRODUCT_NOT_FOUND","INSUFFICIENT_STOCK","EMPTY_ORDER"].includes(e.code) ? 409 : 400;
    ...
    if (req.method === "POST" && url.pathname.match(/^\/api\/orders\/[^/]+\/status$/)) {
      ...
      if (body.status === 'cancelled') {
        ...
        db.prepare("UPDATE orders SET status=?,cancelled_at=NULL WHERE id=?").run(body.status, id);
      }
      ...
      return send(req, res, 200,{ok:true,orderId:id,status:body.status});
    }
    ...
    if (req.method === "POST" && url.pathname === "/api/coupons") {
      ...
      if (c.type === 'percent' && Number(c.value || 0) > 100) return send(req, res, 400, { error: "INVALID_COUPON", message: "Percentage cannot exceed 100%." });
      ...
      db.prepare("INSERT INTO coupons (id,code,type,value,min_subtotal,max_uses,used_count,active,expires_at,description) VALUES (?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET code=excluded.code,type=excluded.type,value=excluded.value,min_subtotal=excluded.min_subtotal,max_uses=excluded.max_uses,used_count=excluded.used_count,active=excluded.active,expires_at=excluded.expires_at,description=excluded.description")
        .run(id, code, c.type || 'percent', Number(c.value || 0), Number(c.minSubtotal || 0), Number(c.maxUses || 0), Number(c.usedCount || 0), c.active === false ? 0 : 1, c.expiresAt ? Number(c.expiresAt) : null, String(c.description || ''));
    ...
    if (req.method === "POST" && url.pathname === "/api/returns") {
      ...
      return send(req, res, 201,{request:returnRow(db.prepare("SELECT * FROM return_requests WHERE id=?").get(id))});
    }
    if (req.method === "POST" && url.pathname.match(/^\/api\/returns\/[^/]+\/status$/)) {
      ...
      const request=db.prepare("SELECT * FROM return_requests WHERE id=?").get(id); if(!request) return send(req, res, 404,{error:'RETURN_NOT_FOUND',message:'Return request not found.'});
      const order=db.prepare("SELECT * FROM orders WHERE id=?").get(request.order_id); db.exec('BEGIN IMMEDIATE');
      try {
      ...
      return send(req, res, 200,{request:returnRow(db.prepare("SELECT * FROM return_requests WHERE id=?").get(id)),migrated:true});
    }
    ...
    console.error(error);
    send(req, res, error.message === "Request body too large" ? 413 : 500, {
      error: "SERVER_ERROR",
...
server.on("error", (error) => {
  ...
  process.exitCode = 1;
  try { db.close(); } catch { /* Shutdown cleanup is best-effort. */ }
  process.exit(1);
});
